	...
	private Tree<Integer> S(Integer... values) throws ParseException {
		switch (lex.curToken().name()) {
			case "FUN" -> {
				...
				lex.nextToken();
				Tree<Integer> RT6 = RT();
				Tree<Integer> t = new Tree<Integer>(new NonTerminal("S"), token1, FN2, token3, FA4, token5, RT6);
				return t;
			}
			default -> throw new IllegalStateException("Unexpected value: " + lex.curToken());
		}
	...
	private Tree<Integer> FAC(Integer... values) throws ParseException {
		switch (lex.curToken().name()) {
			case "," -> {
				...
				Tree<Integer> t = new Tree<Integer>(new NonTerminal("FAC"), token1, token2, token3, T4, FAC5);
				return t;
			}
			case ")" -> {
				Tree<Integer> t = new Tree<Integer>(new NonTerminal("FAC"));
				return t;
			}
			default -> throw new IllegalStateException("Unexpected value: " + lex.curToken());
		}
